...
Publishing a stable GitHub release triggers `.github/workflows/homebrew.yml`. The official `Homebrew/actions/setup-homebrew` action prepares Homebrew, then `brew bump-cask-pr --write-only` updates the cask version and calculates SHA-256 checksums from both macOS DMGs and the Linux x86-64 AppImage. The workflow commits `Casks/tusklet.rb` directly to the default branch of `mattsverse/homebrew-tap`. Drafts and prereleases are excluded. Rerunning an already published version is a no-op; older versions cannot overwrite a newer cask. The tap must allow direct pushes by the publishing token.
...
The first publication seeds the tap checkout from `.github/homebrew/tusklet.rb`. Its bootstrap version and placeholder checksums are replaced by Homebrew before the cask is pushed; subsequent releases update the existing cask in the tap. The workflow skips Homebrew's audit because the macOS app is deliberately ad-hoc signed. No Python tooling is required.
